...
class LoginController extends GetxController {
  ...
  void loginUser() async {
    ...
    //not sure about what this is suppose to do, return 503 every time
    ...
    try {
      await FirebaseAuth.instance
          ...
          .then((value) async {
        //if remember pressed to to dashboard else login screen
        print('xxxxxx$value');
        _saveIsRemembered(remember);
        Get.offAndToNamed(RouteHelper.dashboardScreen);
      });
  ...
  void _saveIsRemembered(bool remember) async {
    if (remember) {
      await loginRepo.apiClient.sharedPreferences
          .setBool(SharedPreferenceHelper.rememberMeKey, true);
    } else {
